...
class Worker(QObject):
    ...
    sqlerror = pyqtSignal()
    sqlcomplete = pyqtSignal()

    def run(self):
        global QueryTextList, data, con, ResultTabCounter

        ttq = len(QueryTextList)
        i = 1
        for QueryText in QueryTextList:
            try:
                """Long-running task."""
                self.progress.emit('Running query {} of {}'.format(i, ttq))
                data = pd.read_sql(QueryText, con)
                data = data.fillna('*Null*')
                self.sqlcomplete.emit()
...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def setupUi(self, MainWindow):
        ...
        self.pushButton_12.clicked.connect(self.ImportQuery)
        ...
        self.tabWidget.currentChanged.connect(self.ExportEnabled)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton_12.setText(_translate("MainWindow", "Import SQL"))
    ...
    def StartRunningQuery(self):
        global QueryTextList, ResultTabCounter, WorkerTabCounter

        self.textEdit.setFocus()
        WorkerTabCounter = ResultTabCounter

        if len(self.textEdit.toPlainText().strip()) == 0:
            print("No SQL provided")
        elif len(self.textEdit.textCursor().selection().toPlainText()) == 0:
            QueryTextList = sqlparse.split(self.textEdit.toPlainText())
            self.CreateWorkerThread()
        else:
            QueryTextList = sqlparse.split(self.textEdit.textCursor().selection().toPlainText())
            self.CreateWorkerThread()
    ...
    def CreateWorkerThread(self):
        ...
        exec("""self.worker_%d.sqlerror.connect(self.DeleteWorkerThread)""" % ResultTabCounter)
        exec("""self.worker_%d.sqlcomplete.connect(self.SetTableView)""" % ResultTabCounter)
        exec("""self.worker_%d.finished.connect(self.DeleteWorkerThread)""" % ResultTabCounter)

        exec("""self.thread_%d.start()""" % ResultTabCounter)

    def DeleteWorkerThread(self):
        global WorkerTabCounter

        try:
            exec("""self.worker_%d.stop()""" % WorkerTabCounter)
            exec("""self.worker_%d.deleteLater()""" % WorkerTabCounter)
            exec("""self.thread_%d.exit(0)""" % WorkerTabCounter)
            exec("""self.thread_%d.deleteLater()""" % WorkerTabCounter)
        except:
            print("no object")
    ...
    def ApplyFilter(self, FilterList=[]):
        ...
        try:
            if datatype == 'Float64':
                df[eval("""str(df.columns[columnnumber])""")] = df[eval("""str(df.columns[columnnumber])""")].astype(
                    np.float64)
    ...
    def ImportQuery(self):
        # dlg = QDialog(self)
        # dlg.setWindowTitle("Import Query!")
        # dlg.exec()
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getOpenFileName(self,"QFileDialog.getOpenFileName()", "","Text Files (*.txt);;SQL "
                                                                                           "Files (*.sql)",
                                                                                            options=options)
        if fileName:
            file = open("Creds.txt", "r+")
            Query = file.read()
            self.textEdit.setText(Query)
...
if __name__ == "__main__":
    ...
    highlight = sh.PythonHighlighter(ui.textEdit.document())
    MainWindow.show()
    sys.exit(app.exec_())
